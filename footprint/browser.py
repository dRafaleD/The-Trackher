...
from utils.helpers import get_file_size, get_dir_size, safe_remove
...
def _clean_sqlite_db(db_path: Path, queries: list[str]) -> tuple[bool, int]:
    ...
    Silme işleminden önce ve sonra dosya boyutunu karşılaştırarak temizlenen boyutu bulur
    (veya VACUUM komutu göndeririz ama veritabanı kilitliyken VACUUM bazen çökebilir).
    Bu yüzden boyut farkını tahmini hesaplar.
    ...
    try:
        ...
        for query in queries:
            try:
                ...
                pass # Tablo yoksa veya kilitliyse atla
        conn.commit()
        # Optional: try to vacuum to reclaim space, but might fail if locked
        try:
        ...
        # Even if vacuum fails and size doesn't change, we wiped data. Return a nominal size if 0.
        return True, freed if freed > 0 else initial_size // 2 
    except sqlite3.Error:
        return False, 0
...
def clean_browser_data(dry_run: bool = False) -> list[dict]:
    ...
    for browser_name, base_path, sub_dirs in _browser_targets():
        ...
        if is_firefox:
            ...
            # Cache dizinleri bazen User Data altında doğrudan da olabilir
            search_bases.append(base_path)
        else:
        ...
        for profile_base in search_bases:
            ...
            for sub_dir in sub_dirs:
                ...
                browser_found = True
                label = f"{browser_name} — {sub_dir if sub_dir != '.' else 'Cache'}"
            ...
            # 2. SQLite Veritabanlarını Temizle (History, Cookies)
            ...
            for filename, queries in sqlite_files:
                ...
                browser_found = True
                label = f"{browser_name} — {filename}"
                size = get_file_size(target_file)
                item = {"path": str(target_file), "size": size, "type": f"{label} (SQLite)"}
                
                if dry_run:
                    print_info(f"[dim]{label} (SQLite)[/dim]  →  İçerik temizlenecek  ({size:,} bayt)")
                    results.append(item)
                else:
